...
<body>
	...
	<form action="amtListAction_toQuery" id="amtList" method="post" name="qryForm" >
		<ul id="mainAction" class="main-actions clearfix">
			...
			<!-- <li>
				<span style="display:inline-block;" class='sch-button' onclick="$('#amtList').submit()">查询</span>&nbsp;
			</li> -->
		</ul>
		<div class="toolBar">
	...
	<div id="scrollDiv" style="width: 100%; overflow-y: auto; height: 100%;">
		<table class="data-content" width="100%" border="1" cellpadding="0" cellspacing="0">
			...
			<tr>
				<td>${rst.date_type}</td>
				<td>${rst.total_income}</td>
				<td>${rst.innerBank_total_income}</td>
				<td>${rst.outerBank_total_income}</td>
				<td>${rst.incr_total_income}</td>
			</tr> 
			</c:forEach>
		...
		<div class="page-tag clearfix">
			<div class="float-l">
				<form action="amtListAction_toQuery1" method="post" name="listForm">
	...
	<form action="amtListAction_fixedAssets" name="detailForm" method="post">
	...
	function selectAll(){
		...
		function amtListAction_toQuery() {
			setTimeout("qryForm.submit()", 100);
			return false;
		}
		
		function amtListAction_fixedAssets() {
