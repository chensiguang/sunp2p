...
<body>
	...
	<form action="borrowerFinanceAction_toQuery"  method="post" name="qryForm">
	...
	<div id="scrollDiv" style="width: 100%; overflow-y: auto; height: 100%;">
		...
		<c:forEach items="${page.result}" var="rst">
			<tr>		
				...
				<td>${rst.cus_type}</td>
				<td>${rst.name}</td>
				<td>${rst.on_passage_count}</td>
				<td>${rst.on_passage_amt}</td>
				<td>${rst.succ_finance_count}</td>
				<td>${rst.succ_finance_amt}</td>
			</tr> 
		</c:forEach>
	...
	<div class="page-tag clearfix">
		<div  class="float-l">
			<form action="borrowerFinanceAction_toQuery" method="post" name="listForm">
	...
	<form action="borrowerFinanceAction_fixedAssets" name="detailForm" method="post">
	...
	$(document).ready(function() {
		...
		$.ajax({
			type : "post",
			url : "borrowerFinanceAction_getButton",
			dataType : "text",
			timeout: 6000,
			cache : false,
			error : function(msg) {
	...
	function selectAll(){
		...
		function borrowerFinanceAction_toQuery() {
			setTimeout("qryForm.submit()", 100);
			return false;
		}
		
		function borrowerFinanceAction_fixedAssets() {
